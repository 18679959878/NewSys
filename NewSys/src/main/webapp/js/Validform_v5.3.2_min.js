...
(function(d, f, b) {
	...
	a.util = {
		...
		getValue : function(m) {
			...
			l = d.trim(l);return a.util.isEmpty.call(m, l) ? "" : l
		},
			...
			o.find("[datatype]").each(function() {
				if (l == 2) {
					if (d(this).parent().next().find(".Validform_checktip").length == 0) {
						d(this).parent().next().append("<span class='Validform_checktip' />");d(this).siblings(".Validform_checktip").remove()
					}
				} else {
					if (l == 3 || l == 4) {
		...
		usePlugin : function(o, l, n, r) {
			...
			if (r != "addRule" && o.jqtransform && d.fn.jqTransSelect) {
				...
				if (o.jqtransform.selector) {
					s.find(o.jqtransform.selector).filter('input:submit, input:reset, input[type="button"]').jqTransInputButton();s.find(o.jqtransform.selector).filter("input:text, input:password").jqTransInputText();s.find(o.jqtransform.selector).filter("input:checkbox").jqTransCheckBox();s.find(o.jqtransform.selector).filter("input:radio").jqTransRadio();s.find(o.jqtransform.selector).filter("textarea").jqTransTextarea();
		...
		getNullmsg : function(o) {
			...
			if (l != "") {
				k = k.replace(/\{0\|(.+)\}/, l);
				if (n.attr("recheck")) {
					k = k.replace(/\{(.+)\}/, "");n.attr("nullmsg", k);return k
				}
			} else {
		...
		getErrormsg : function(s, n, u) {
			var o = /^(.+?)((\d+)-(\d+))?$/,
				...
				r;
			if (u == "recheck") {
				r = s.data("tipmsg").reck || e.reck;return r
			}
			...
			for (var k in p) {
				if (k.indexOf(q[1]) != -1 && m.test(k)) {
					...
					s.data("tipmsg").w[q[0]] = r;return r
				}
			}
						...
						if (!m) {
							...
							for (var B in a.util.dataType) {
								...
								if (m[1] === z[1]) {
									...
									a.util.dataType[t] = new RegExp(w, r);break
								}
							}
						}
					}
		...
		showmsg : function(n, l, m, k) {
			...
			if (typeof l == "function") {
				l(n, m, a.util.cssctl);return
			}
			if (l == 1 || k == "byajax" && l != 4) {
				j.find(".Validform_info").html(n)
			}
			if (l == 1 && k != "bycheck" && m.type != 2 || k == "byajax" && l != 4) {
				i = false;j.find(".iframe").css("height", j.outerHeight());j.show();h(j, 100)
			}
			if (l == 2 && m.obj) {
		...
		check : function(v, t, n) {
			...
			if (q && !a.util.isEmpty.call(d(this), k) && !n) {
				...
				m[0].validform_ckvalue = k;a.util.showmsg.call(v, v.data("tipmsg").c || e.c, o.tiptype, {
					obj : m,
					type : 1,
					sweep : o.tipSweep
				...
				var p = {
					...
					success : function(x) {
						if (d.trim(x.status) === "y") {
							m[0].validform_valid = "true";x.info && m.attr("sucmsg", x.info);a.util.showmsg.call(v, m.attr("sucmsg") || v.data("tipmsg").r || e.r, o.tiptype, {
								obj : m,
								type : 2,
								sweep : o.tipSweep
							}, "bycheck");r.removeClass("Validform_error");
							g = null;
							if (m[0].validform_subpost == "postform") {
								v.trigger("submit")
							}
						} else {
							m[0].validform_valid = x.info;a.util.showmsg.call(v, x.info, o.tiptype, {
								obj : m,
								type : 3,
								sweep : o.tipSweep
					...
					error : function(x) {
						...
						r[0].validform_ajax = null;return true
					}
				};
				if (u.success) {
					var w = u.success;
				...
				r[0].validform_ajax = d.ajax(u);return "ajax"
			} else {
				...
				if (d(this).attr("ajaxurl") && !a.util.isEmpty.call(d(this), z)) {
					if (this.validform_valid !== "true") {
						...
						a.util.showmsg.call(w, w.data("tipmsg").v || e.v, o.tiptype, {
							...
							type : 3,
							sweep : o.tipSweep
						});A.addClass("Validform_error");y.trigger("blur", [ "postform" ]);
						if (!o.showAllError) {
							s = false;return false
						}
						s && (s = false);return true
			...
			if (s) {
				...
				if (o.ajaxPost || r === "ajaxPost") {
					...
					var p = {
						...
						success : function(y) {
							if (d.trim(y.status) === "y") {
								w[0].validform_status = "posted";a.util.showmsg.call(w, y.info, o.tiptype, {
									obj : w,
									type : 2,
									sweep : o.tipSweep
								}, "byajax")
							} else {
								w[0].validform_status = "normal";a.util.showmsg.call(w, y.info, o.tiptype, {
									obj : w,
									type : 3,
									sweep : o.tipSweep
								}, "byajax")
						...
						error : function(y) {
							...
							a.util.showmsg.call(w, z, o.tiptype, {
								...
								type : 3,
								sweep : o.tipSweep
							}, "byajax");o.callback && o.callback(y);
							w[0].validform_status = "normal";
							w[0].validform_ajax = null
						}
					};
	...
	a.prototype = {
		...
		resetForm : function() {
			var k = this;
			a.util.resetForm.call(d(k.forms));return this
		},
		abort : function() {
			var k = this;
			d(k.forms).each(function() {
		...
		config : function(k) {
			var l = this;
			k = k || {};d(l.forms).each(function() {
				var m = d(this);
				this.settings = d.extend(true, this.settings, k);a.util.enhance.call(m, this.settings.tiptype, this.settings.usePlugin, this.settings.tipSweep)
			});return this
		}
	};
